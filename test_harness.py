...
import numpy as np  # <-- BARIS INI YANG DITAMBAHKAN UNTUK MEMPERBAIKI ERROR
...
def append_results_to_csv(results_list: list, run_id: int, algorithm: str, dataset: str):
    ...
    # Handle case where no tasks succeed and start_time might be missing
    valid_start_times = [r['start_time'] for r in results_list if r['start_time']]
    if not valid_start_times: return # Cannot process if no valid start times
    ...
    for r in results_list:
        new_r = r.copy()
        new_r['run_id'] = run_id
        new_r['algorithm'] = algorithm
        new_r['dataset'] = dataset
        if r['start_time']:
...
def main(algorithm: str, dataset_type: str):
    ...
    if not dataset_path:
        print(f"Error: Tipe dataset '{dataset_type}' tidak valid.")
        return
    ...
    if all_makespans:
        ...
        print(f"Rata-rata Makespan: {np.mean(all_makespans):.4f} detik")
        print(f"Makespan Terbaik  : {np.min(all_makespans):.4f} detik")
        print(f"Makespan Terburuk : {np.max(all_makespans):.4f} detik")
        print(f"St. Deviasi       : {np.std(all_makespans):.4f} detik")
...
if __name__ == "__main__":
    ...
    if os.path.exists(RESULTS_FILE):
        print(f"Menghapus file hasil lama: {RESULTS_FILE}")
